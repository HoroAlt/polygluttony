...
mod tests {
    ...
    fn signal_punctuation_counts_question_mismatches() {
        ...
        let (score, first, flagged) = crate::validation::drift::signals::punctuation(&p);
        assert!(score > 0.0);
    ...
    fn signal_length_ratio_flags_outliers() {
        ...
        let (score, first, _) = crate::validation::drift::signals::length_ratio(&p);
        assert!(score > 0.0);
        assert_eq!(first, Some(1));
    }
    ...
    fn signal_glossary_position_detects_missing_terms() {
        ...
        let (score, first, _) = crate::validation::drift::signals::glossary_position(&p, &terms);
        assert!(score > 0.0);
        assert_eq!(first, Some(1));
    }
